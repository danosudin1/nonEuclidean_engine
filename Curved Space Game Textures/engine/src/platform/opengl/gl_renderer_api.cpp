...
{
	int i = GL_MAX_TESS_PATCH_COMPONENTS;
...
void engine::renderer_api::draw_indexed_patches_two(const ref<vertex_array>& vertex_array)
{
	glLineWidth(1.5f);
	glPatchParameteri(GL_PATCH_VERTICES, 2);
	glDrawElements(GL_PATCHES, vertex_array->index_buffer()->count(), GL_UNSIGNED_INT, (void*)0);
}

void engine::renderer_api::draw_indexed_patches(const ref<vertex_array>& vertex_array)
{
	glLineWidth(1.5f);
