...
engine::gl_shader::gl_shader(const std::string& name, const std::string& vertex_file_path, const std::string& geometry_file_path,
	const std::string& tess_control_file_path, const std::string& tess_evaluation_file_path, const std::string& fragment_file_path)
    : m_name(name)
{
    std::unordered_map<uint32_t, std::string> sources;

	sources[GL_VERTEX_SHADER] = read_file(vertex_file_path);
	if (!geometry_file_path.empty())
		sources[GL_GEOMETRY_SHADER] = read_file(geometry_file_path);
	if (!tess_control_file_path.empty())
		sources[GL_TESS_CONTROL_SHADER] = read_file(tess_control_file_path);
	if (!tess_evaluation_file_path.empty())
		sources[GL_TESS_EVALUATION_SHADER] = read_file(tess_evaluation_file_path);
	if (!fragment_file_path.empty())
		sources[GL_FRAGMENT_SHADER] = read_file(fragment_file_path);
    compile(sources);
}
...
{ 
	...
	//LOG_CORE_ERROR("[shader] set_uniform (float) (prog {0}): uniform: '{1}'", m_program_id, name);
} 
...
void engine::gl_shader::set_uniform(const std::string& name, const glm::mat4& mat) 
{
	...
	//LOG_CORE_ERROR("[shader] set_uniform (glm::mat4) (prog {0}): uniform: '{1}'", m_program_id, name);
}
