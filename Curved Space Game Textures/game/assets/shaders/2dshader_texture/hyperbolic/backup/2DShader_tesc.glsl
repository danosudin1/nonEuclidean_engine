...
void tesselation(float delta_theta)
{
	...
	float cosh_edge = cs_cosh_r[0] * cs_cosh_r[1] - cs_sinh_r[0] * cs_sinh_r[1] * cos_delta_theta;

	float edge = acosh(cosh_edge);

	//float sinh_edge = sinh(es_edge[gl_InvocationID]);

	//es_cos_alpha[gl_InvocationID] = ((cs_cosh_r[0] * cosh_edge) - cs_cosh_r[1]) / (cs_sinh_r[0] * sinh_edge);
}
...
void main()  
{
	if (!(gl_InvocationID==2))
	{
		...
		es_final_direction[gl_InvocationID] = 0;
	}
	else
	{
		...
		if(!(cs_position[0].x == 0.0f || cs_position[1].x == 0.0f)) 
		{
			...
			es_final_direction[gl_InvocationID] = direction;
